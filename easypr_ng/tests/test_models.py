...
class Easypr_ngTest(TestCase):
	...
	# def create_marketing_bundle(self, name_slug = "test-marketing-bundle",name="test marketing bundle",features ="test; media; caption"):
	# 	return Media_Marketing_Bundles.objects.create(name_slug = name_slug, name = name, features = features)

	# def test_marketing_bundles(self):
	# 	mb = self.create_marketing_bundle()
	# 	self.assertTrue(isinstance(mb, Media_Marketing_Bundles))
	# 	self.assertEqual(tm.__unicode__(), tm.media_type)
